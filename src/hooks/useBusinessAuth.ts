...
const ctx = createContext<BusinessAuthContext | null>(null);
...
export function BusinessAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  return <ctx.Provider value={{ business, token, login, signup, logout, loading }}>{children}</ctx.Provider>;
}
